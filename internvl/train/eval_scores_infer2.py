...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        if train_eval=='infer':
            self.raw_data = []
            input_path = meta.get('annotation_infer')
            img_prompt = meta.get('img_prompt')
            with open(img_prompt, 'r') as f:
                img_prompt_list = f.readlines()
            if not input_path:
                raise ValueError("'annotation_infer' path not provided in meta file for 'infer' mode.")
            ...
            if os.path.isdir(input_path):
                ...
                logger.info(f"Reading image paths from TXT file: {input_path}")
                self.root = meta.get('root', os.path.dirname(input_path))
                logger.info(f"Using image root: {self.root}")
                with open(input_path, 'r') as f:
                    image_files = [line.strip() for line in f if line.strip()]
            else:
                raise ValueError(f"Unsupported file type for 'annotation_infer': {input_path}. Must be a directory or a .txt file.")

            for i, img_path in enumerate(image_files):
                prompt = img_prompt_list[i].strip()
                data_obj = {
                    "image": img_path,
                    "mos": 0.0,  # Use a dummy MOS score since it's not available
                    "conversations": [
                        {"from": "human", "value": f"How would you rate the correspondence of this image and its prompt? prompt: {prompt}"},
                        {"from": "gpt", "value": "The correspondence of the image and its prompt is excellent."}
                    ]
                }
                self.raw_data.append(json.dumps(data_obj))
...
def save_and_evaluate(data_list, output_file='results.csv'):
    ...
    df = pd.DataFrame(data_list, columns=['imgname','pred_score'])
...
def main():
    ...
    eval_dataset = build_datasets(
        data_args, 'infer', tokenizer, tcs_loader, model, group_by_length=training_args.group_by_length,
    ...
    with torch.no_grad():
        for item2 in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            for k, v in item2.items():
                if k!='imgname' and k!='answer':
                    item[k] = v.to(model.device)
            # Get video name
            imgname = item2['imgname'][0]  # Assuming video name is part of the data
